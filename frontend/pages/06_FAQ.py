...
layout = html.Div([
    ...
    html.Div([
        ...
        html.Hr(),
         dbc.Button(
            "1. 정량지표와 정성지표란?",
            id="Q1",
            className="mb-3 text-start text-body",
            color="secondary",
            n_clicks=0,
        ),
        dbc.Collapse(
        ...
        dbc.Collapse(
            dcc.Markdown(
                ...
                - **간단한 연산을 하는 일종의 새로운 모델을 이용해 추천 결과물을 만들어 내는 과정이기 때문에 _시간이 꽤 소요되는 작업입니다._**
                
                - Reranking 전 후의 정량, 정성 지표들과 추천 결과물을 비교하여 원하는 정도의 Reranking을 설정하는 $\\alpha$를 조절하며 자신만의 모델을 만들어보세요!
    ...
    ], className="container vstack gap-2", style={"margin-top": "4rem"}
    ),
])
