...
model_form = html.Div(
    [
        ...
        dcc.Dropdown(
            id="selected_model_by_name", placeholder="Reranking 진행할 실험을 선택하세요"
        ),
        html.Hr(),
        html.H6(
            ["Reranking 파라미터 α ", html.Span("�", id="alpha-tooltip")], className="mb-0"
        ),
        dbc.Tooltip(
            [
                "일종의 가중치이며, 낮을수록 Reranking 모델의 추천이 많이 반영됩니다.",
                ...
                [
                    ...
                    html.Div(
                        [
                            ...
                            dbc.Col(
                                dbc.Spinner(
                                    html.Div(id="rerank_dist_fig"), color="primary"
                                )
...
layout = html.Div(
    [
        ...
        html.Div(
            [sidebar, total_graph, html.Div(id="rerank_specific_metric_children")],
            className="content",
        ),
        # html.Div(id='trash2'),
...
def get_stored_selected_models(exp_names: str, store, user, dataset) -> pd.DataFrame:
    ...
    params = {"ID": user["username"], "dataset_name": dataset, "exp_ids": [exp_id]}
    ...
    params = {"ID": user["username"], "dataset_name": dataset, "exp_names": [exp_names]}
    ...
    rerank_total_metrics = rerank_total_metrics.drop(
        ["experiment_name"], axis=1
    )  # , 'alpha'
    rerank_total_metrics = pd.concat([exp_metrics, rerank_total_metrics], axis=0)

    rerank_total_metrics_users = pd.DataFrame().from_dict(
        a["user_metrics"], orient="tight"
    )
    rerank_total_metrics_users.index = rerank_total_metrics.index[1:]
    rerank_total_metrics_users = pd.concat(
        [exp_metrics_users, rerank_total_metrics_users], axis=0
    )
...
def plot_total_metrics(data, n, obj_funcs, state, store):  # df:pd.DataFrame
    if state == 0:
        return html.Div([]), dbc.Alert(
            "Rerank 버튼을 눌러 리랭킹 된 실험들의 지표를 확인해보세요!", color="info"
        )
        # html.Div([
        ...
        store_df = pd.DataFrame(store).set_index("experiment_name")
        tmp_metrics = rerank_total_metrics.drop(
            ["diversity_jac", "serendipity_pmi", "alpha"], axis=1
        )
        ...
        for i, obj_fn in enumerate(tmp_metrics.index):  # data
            ...
            fig.add_bar(
                name=obj_fn,
                x=metrics,
                y=list(tmp_metrics.loc[obj_fn, :]),
                text=list(tmp_metrics.loc[obj_fn, :]),
        ...
        child = html.Div(
            dbc.Row(
                [
                    ...
                    dbc.Col(
                        dbc.Alert(
                            "아래 그래프를 통해 어떤 Reranking 방식을 Deep Analysis에 사용할지 결정하세요!",
                            color="warning",
                            style={
                                "width": "700px",
                                "margin-right": "0",
    ...
    if sort_of_metric == "Qual":
        ...
        for i, obj_fn in enumerate(qual_metrics.index):  # data
            ...
            fig.add_bar(
                name=obj_fn,
                x=metrics,
                y=list(qual_metrics.loc[obj_fn, :]),
                text=list(qual_metrics.loc[obj_fn, :]),
            )  # , marker={'color' : colors[i]}
        ...
        for i, obj_fn in enumerate(quant_metrics.index):  # data
            ...
            fig.add_bar(
                name=obj_fn,
                x=metrics,
                y=list(quant_metrics.loc[obj_fn, :]),
                text=list(quant_metrics.loc[obj_fn, :]),
            )  # , marker={'color' : colors[i]}
